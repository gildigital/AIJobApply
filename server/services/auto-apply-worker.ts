...
async function processJob(queuedJob: JobQueue): Promise<void> {
  try {
    ...
    const delayMs = APPLY_DELAY_MS[userPlan as keyof typeof APPLY_DELAY_MS] || APPLY_DELAY_MS.FREE;
    await new Promise(resolve => setTimeout(resolve, delayMs));
    
  } catch (error) {
    console.error(`[Auto-Apply Worker] Error processing job ${queuedJob.id}:`, error);
 ...
 */
export async function enqueueJobsForUser(userId: number, jobIds: number[]): Promise<JobQueue[]> {
  ...
  try {
    ...
    // Add jobs to queue
    const queuedJobs = await storage.enqueueJobs(queueItems);
    ...
    console.error(`[Auto-Apply Worker] Error enqueuing jobs for user ${userId}:`, error);
    throw error;
  }
}
